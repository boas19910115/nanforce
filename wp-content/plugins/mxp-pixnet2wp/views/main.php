...
if (isset($re_download_img) && isset($enable_log) && isset($complete_remove) && isset($post_status) && isset($comment_status) && isset($ping_status) && isset($post_type) && isset($post_tags) && isset($pixnet_account) && isset($post_cate) && isset($post_author) && isset($cm_admin_name) && isset($cm_admin_email)) {
...
<p>對於此項服務，你的身份為：【<?php echo $pay_user == true ? "<font color=red>已授權進階用戶</font>" : '<a href="https://mxp.tw/lw" target="blank">一般用戶</a>'; ?>】</p>
<?php if ($pay_user != true): ?>
<p>對於「一般用戶」與「進階授權」用戶的差異，僅有授權用戶會於匯入時包含痞客邦標籤以及留言內容，藉此在搜尋引擎優化上得到方便的轉換。若你覺得這是一項重要的功能，請參考「<a href="https://goo.gl/Kt41pF" target="blank">申請使用進階版 Pixnet 痞客邦匯入工具</a>」，提出申請。<strong>注意：一個痞客邦網站對應一個轉移網域，如有測試需求請聯絡作者，否則無法提供授權轉移，請再次購買授權。</strong></p>
<?php endif;?>
<p><strong>目前用量（匯入篇數）：</strong><?php echo get_option("mxp_pixnet2wp_usage", "0"); ?> / <?php echo get_option("mxp_pixnet2wp_post_quota", "0"); ?>（用量限制僅為避免過度使用，如果有超量需求歡迎<a href="https://goo.gl/y2dcqL" target="blank">聯絡作者</a>，免費增加！）</p>
<?php endif;?>
<h2>網站預設資訊</h2>
<p>匯入指定發文使用者：
...
<p>迴響中管理員顯示名稱：
<input type="text" name="mxp_pixnet2wp_post_comment_admin_displayname" value="<?php echo get_option("mxp_pixnet2wp_post_comment_admin_displayname", "版主"); ?>"></input>【<?php echo $pay_user == true ? "<font color=red>已授權</font>" : '<a href="https://mxp.tw/lw" target="blank">付費用戶功能</a>'; ?>】
</p>
<p>迴響中管理員顯示信箱：
<input type="text" name="mxp_pixnet2wp_post_comment_admin_email" value="<?php echo get_option("mxp_pixnet2wp_post_comment_admin_email", ""); ?>"></input>【<?php echo $pay_user == true ? "<font color=red>已授權</font>" : '<a href="https://mxp.tw/lw" target="blank">付費用戶功能</a>'; ?>】
</p>
<p>允許通告：
...
<p>匯入文章標籤：
<input type="text" name="mxp_pixnet2wp_post_tags" size="30" value="<?php echo get_option("mxp_pixnet2wp_post_tags", ""); ?>" />（逗點（,）分隔）【<?php echo $pay_user == true ? "<font color=red>已授權</font>" : '<a href="https://mxp.tw/lw" target="blank">付費用戶功能</a>'; ?>】
</p>
<h2>開發者功能</h2>
<p>刪除外掛時連帶全部設定資料：
